...
def get_data_from_sim(robot, motor_id_list):
	...
	# w_base = R * w_world
	imu_data[7] = get_matrix[0] * get_velocity[1][0] + get_matrix[1] * get_velocity[1][1] + get_matrix[2] * \
	              get_velocity[1][2]
	imu_data[8] = get_matrix[3] * get_velocity[1][0] + get_matrix[4] * get_velocity[1][1] + get_matrix[5] * \
	              get_velocity[1][2]
	imu_data[9] = get_matrix[6] * get_velocity[1][0] + get_matrix[7] * get_velocity[1][1] + get_matrix[8] * \
	              get_velocity[1][2]
...
#         pre_pos[i] = foot_pos
def write_input(f, quad):
	bse, orientation = p.getBasePositionAndOrientation(quad)
	linear_vel, angular_vel = p.getBaseVelocity(quad)
	joint_pos = []
	joint_vel = []
	for j in range(12):
		pos = p.getJointState(quad, motor_id_list[j])[0]
		vel = p.getJointState(quad, motor_id_list[j])[1]
		joint_pos.append(pos)
		joint_vel.append(vel)
	tmp = list(orientation) + list(linear_vel) + list(angular_vel) + list(joint_vel) + list(joint_pos)
	# print(len(list(orientation)),len(list(linear_vel)),len(list(angular_vel)),len(list(joint_vel)),len(list(joint_pos)))
	tmp_round = [round(i, 6) for i in tmp]
	f.write(str(tmp_round))
	f.write('\n')
	return


def write_output(f, quad):
	joint_pos = []
	for j in range(12):
		pos = p.getJointState(quad, motor_id_list[j])[0]
		joint_pos.append(pos)
...
def main():
	...
	mode = 'plane'
	print(mode)
	# f_input=open('data_extract/input/'+mode+'_input.txt','w')
	# f_input.truncate(0)
	# f_output=open('data_extract/output/'+mode+'_output.txt','w')
	# f_output.truncate(0)
	while 1:
...
if __name__ == '__main__':
	...
	liner_x = p.addUserDebugParameter("liner_x", -5, 5, 0)
	liner_y = p.addUserDebugParameter("liner_y", -5, 5, 0)
	...
	robot = p.loadURDF("bilibili/quadruped_model/a1/a1.urdf", [0, 0, 0.4], useFixedBase=False)
